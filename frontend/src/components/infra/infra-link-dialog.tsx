...
import { Server, Search, Loader2, MapPin } from 'lucide-react';
import { useInfraItems, useLinkInfra, useInfraByEntity } from '@/lib/hooks/use-infra';
...
interface InfraLinkDialogProps {
    open: boolean;
...
/**
 * Standalone dialog to search and link infra items to a finding, testcase, or note.
 * Used from the engagement detail page action menus.
 */
export function InfraLinkDialog({ open, onOpenChange, entityType, entityId }: InfraLinkDialogProps) {
    const [search, setSearch] = useState('');
    const linkInfra = useLinkInfra();
    const { data: linkedItems = [] } = useInfraByEntity(entityType, entityId);
