...
import { Radar, Plus, X, ExternalLink, Search, Loader2, Unlink } from 'lucide-react';
import { useIntelByEntity, useIntelItems, useLinkIntel, useUnlinkIntel, IntelItem } from '@/lib/hooks/use-intel';
...
interface IntelLinkSectionProps {
    entityType: 'finding' | 'testcase' | 'note';
    entityId: string;
}

export function IntelLinkSection({ entityType, entityId }: IntelLinkSectionProps) {
    const { data: linkedItems = [], isLoading } = useIntelByEntity(entityType, entityId);
    ...
    const handleUnlink = async (itemId: string) => {
        try {
            await unlinkIntel.mutateAsync({ itemId, entityType, entityId });
            toast.success('Intel unlinked');
        } catch {
            toast.error('Failed to unlink intel');
        }
    };
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-3">
                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Linked Intel</h4>
                <Button
                    variant="ghost"
                    size="icon"
                    className="h-6 w-6 text-cyan-400 hover:text-cyan-300 hover:bg-cyan-500/10"
                    onClick={() => { setPickerOpen(true); setSearch(''); }}
                    title="Link Intel Item"
                >
            ...
            {isLoading ? (
                ...
                <div className="space-y-2">
                    {linkedItems.map(item => (
                        <div
                            key={item.id}
                            className="group flex items-center gap-2 p-2 bg-slate-950/40 rounded-lg border border-slate-800/60 hover:border-cyan-500/30 transition-colors"
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    ...
                                    {item.cve_id && (
                                        <span className="text-[9px] font-mono text-red-400">{item.cve_id}</span>
                                    )}
                                </div>
                            </div>
