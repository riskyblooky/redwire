...
LDAP_KEYS = [
    ...
    "ldap_email_attribute", "ldap_fullname_attribute", "ldap_tls_enabled",
    "ldap_tls_ca_cert",
]
SAML_KEYS = [
...
async def get_auth_settings(
    ...
    ldap = LdapSettings(
        ...
        tls_enabled=raw.get("ldap_tls_enabled", "true").lower() == "true",
...
async def update_ldap_settings(
    ...
    mapping = {
        ...
        "ldap_tls_enabled": str(settings.tls_enabled).lower(),
    }
...
async def test_ldap(
    ...
    raw = await _get_all_settings(db)
    result = test_ldap_connection(raw)  # Reuses keys prefixed with ldap_

    # Also pass the unprefixed keys for the ldap module
    settings_for_test = {
        "server_url": raw.get("ldap_server_url", ""),
        "bind_dn": raw.get("ldap_bind_dn", ""),
    ...
    result = test_ldap_connection(settings_for_test)
    return result
